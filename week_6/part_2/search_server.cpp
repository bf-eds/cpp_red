...
{
    UpdateDocumentBaseSingle(document_input);
//    queries_.push_back(async(launch::async, &SearchServer::UpdateDocumentBaseSingle, this, ref(document_input)));
}
...
void SearchServer::UpdateDocumentBaseSingle(istream &document_input)
{
    ...
    index_.GetAccess().ref_to_value = move(new_index);
}

void SearchServer::AddQueriesStream(istream &query_input, ostream &search_results_output)
{
    queries_.push_back(async(launch::async, &SearchServer::AddQueriesStreamSingle, this, ref(query_input),
                             ref(search_results_output)));
}

void SearchServer::AddQueriesStreamSingle(istream &query_input, ostream &search_results_output)
{
    vector<size_t> docid_count(index_.GetAccess().ref_to_value.GetDocsCount());
    ...
    for (string current_query; getline(query_input, current_query);)
    {
        ...
        {
            for (const auto[docid, hintpoint] : index_.GetAccess().ref_to_value.Lookup(word))
            {
        ...
        docid_count.assign(index_.GetAccess().ref_to_value.GetDocsCount(), 0);
    }
}
